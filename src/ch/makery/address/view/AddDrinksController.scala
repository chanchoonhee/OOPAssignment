				...
				if (nullChecking(drinksIDField.text.value))
					...
					if (nullChecking(drinksNameField.text.value))
						errorMessage += "No valid name!\n"
						  ...
						  print("\nchecked 1")
						  if(MainApp.selectedDrinks != null){
							    if(drinksNameField.text.value.equalsIgnoreCase(MainApp.selectedDrinks.name.value)){
							      print("\ndo nothing")
							    }
							    else if(drinksNameField.text.value.equalsIgnoreCase(items.name.value)){
							        print("\ncheched 3")
                          errorMessage += "Drinks Name Already Exist!\n"
                          }
                                  
							    
							  }else
						    if(drinksNameField.text.value.equalsIgnoreCase(items.name.value)){
							        print("\ncheched 3")
                          errorMessage += "Drinks Name Already Exist!\n"
                          }
						  }
						if (nullChecking(drinksPriceField.text.value))
							errorMessage += "No valid price!\n"
						else if(MainApp.isDouble(drinksPriceField.text.value.toString) == false)
									errorMessage += "No valid price!\n"
